...
data = pd.read_excel("dimxreg.xlsx",sheet_name=0)
criteria=[]
regions=[]
group1=[]
...
component_matrix = pd.DataFrame(pca.components_.T, columns=['PC1','PC2','PC3'], index=group1)

coeffs = pca.components_.T / np.sqrt(exp_var)

coeffs_matrix = pd.DataFrame(coeffs, columns=['PC1','PC2','PC3'], index=group1)
